...
public class A3
{
	...
	public static void main( String args[] ) throws Exception
	{
		...
		if(args.length < 1){
			System.out.println("Less than one argument was used. Re-run the program using the file name and file. Ex: java A3 A3input.txt");
			System.exit(0);
		}
		...
		while(file.hasNext())
		{
			while(file.hasNextInt())
			{
				...
				count++;
				insertInOrder(array, count, array[count-1]);
			}
	...
	static void insertInOrder( int[] arr, int count, int newVal)
	{
		...
		// Why do we need a seperate counter exactly?
